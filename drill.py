...
def split_data(df, target_col="churned", test_size=0.2, random_state=42):
    ...
    """
    # Separate features (X) and target (y), then split with stratification
    X = df.drop(target_col, axis=1) #input should'nt has target-col
    y = df[target_col] # output what is the model's predicits
